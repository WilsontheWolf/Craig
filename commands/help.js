...
const main = (data) => {
    {
        if (!args[0]) {
            const embed = new Discord.MessageEmbed()
                ...
                .setDescription(`My prefix here is \`${data.settings.prefix}\`. For more help on a command run \`${data.settings.prefix}help [command]\`.`)
                .setColor('RANDOM');
            ...
            embed.addFields(Object.keys(values).map(v => {
                return {
                    name: `${v}:`,
                    value: values[v].map(c => `**${data.settings.prefix}${c.name} ${c.usage}**
${c.description}`).join('\n'),
                    inline: true
                };
            }));
            ...
            if (!cmd) return data.reply(`No command found called \`${args[0]}\`. To see all the commands run \`${data.settings.prefix}help\`.`, { allowedMentions: { users: [data.author.id] } });
            const embed = new Discord.MessageEmbed()
                .setTitle(`${cmd.name}:`)
