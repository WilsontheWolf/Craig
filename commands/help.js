...
${c.description}`).join('\n'),
        ...
        if (!cmd) return `No command found called \`${command}\`. To see all the commands run \`${slash ? '/' : data.settings.prefix}help\`.`;
        const embed = new Discord.MessageEmbed()
            .setTitle(`${cmd.name}:`)
...
module.exports.run = async (client, message, args, level) => {
    ...
    if (typeof res === 'string')
        await message.channel.send(res, { allowedMentions: { users: [message.author.id] } });
    else
...
module.exports.slash = {
    ...
    run: async (client, args, data, reply) => {
        await reply(main(args.get('command'), true, data));
    }
};